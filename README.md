...
2. **Jerarquía Semántica Estricta:**
   ...
   - `<section id="proyectos">`: Artículos `<article>` independientes para proyectos de software (*AeroSync Cloud: Plataforma de Telemetría IoT e Inventarios Críticos* y *QuantumPay Gateway: Motor Idempotente de Transacciones Seguras*), con marcas temporales legibles por máquinas (`<time datetime="YYYY-MM-DD">`), tecnologías utilizadas y enlaces seguros externos con `target="_blank"` y `rel="noopener noreferrer"`.
